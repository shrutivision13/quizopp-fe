...
const Header = () => {
  ...
  // Helper function to check if the current path is a back path
  ...
  // Helper function to check if it's the init header
  const initHeader = useMemo(() => ["/get-started", "/start-quiz"].includes(path), [path]);
          ...
          <div className="flex flex-row items-center flex-1 pl-16">
            ...
            <div className="py-10">
              <h1 className="text-14 font-bold dark:text-CFFFFFF">
                {path === "/contests" || path.includes("/join-contest") || path.includes("/contest-rank") ? (
  ...
  if (isMainHeader ) {
    return (
      <header className="h-60 fixed z-99 max-w-maxW top-0 w-full duration-350 bg-C26284C !mt-0">
        <div className="flex flex-row justify-between items-center h-full px-20">
          <div data-testid="side-menu-button" className="py-14 pr-8 flex justify-center cursor-pointer" onClick={() => setIsMenuVisible(true)}>
            <img alt="menu" loading="lazy" width="20" height="20" src={menuIcon} />
          </div>
          ...
          <div className="flex items-center">
            {userData?.isRegister ? (
              ...
              <button onClick={() => navigate("/login")} className="items-center justify-center whitespace-nowrap text-CFFFFFF bg-C0DB25B py-5 px-20">
                LOGIN
              </button>
            )}
