...
import { ApiGetQuizParticipationResult } from "../../../api-wrapper/games/ApiGames";
...
const QuizBattlesEndQuiz = () => {
    ...
    useEffect(() => {
        const participantId = location?.state?.result?._id;
        if (participantId) {
                    ...
                    <div>
                        <div className="mb-14">
                            {/* Skeleton for Calculating result*/}
                            {
                                loading ?
                                    ...
                                    <div class="px-20 font-black mt-24 flex flex-col items-center justify-center text-CE63737">
                                        <p class="mb-4 uppercase font-bold text-[24px] style_enlarge_text__0eTO9">You {results?.user?.result}</p>
                                        <p class="text-C8789C3 uppercase font-bold style_shrink_text__1kxLI">{results?.category}</p>
                                    </div>
                            }
                            ...
                            <div className="flex flex-col item-center justify-center">
                                <div className="mt-14 px-20 w-full flex items-center justify-between">
                                    ...
                                    <QuizBattlesOpponentStat
                                        ...
                                        opponentName={results?.user?.userName}
                                        isWinner={results?.user?.result?.toLowerCase() === 'win'}
                                        isLoading={loading}
                                    />
                                    <div>
                                        <img
                                    ...
                                    {/* Opponent */}
                                    <QuizBattlesOpponentStat
                                        score={results?.opponent?.score}
                                        opponentImage={location?.state?.userImage}
                                        opponentName={results?.opponent?.userName}
                                        statColor="text-CA96DFF"
                                        isWinner={results?.opponent?.result?.toLowerCase() === 'win'}
                                        isLoading={loading}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="relative bg-C20213F flex items-center justify-between px-20 w-full gap-18 pt-36 mb-18 mt-14 animate__animated animate__slideInUp">
                                ...
                                {
                                    loading ? <GameStateSkeleton /> :
                                        <GameStats
                                            accuracy={results?.user?.correctAnswer}
                                            accuracyPercentage={results?.user?.accuracy}
                                            speed={results?.user?.totalSeconds}
                                            speedPercentage={results?.user?.speed}
                                            lifelinesUsed={results?.user?.lifelieUsed}
                                            lifelinesUsedPercentage={results?.user?.lifelinePercentage}
                                        />
                                }
                                {
                                    loading ? <div className="flex flex-col gap-20 self-baseline">
                                ...
                                }
                                {
                                    loading ? <GameStateSkeleton /> :
                                        <GameStats
                                            accuracy={results?.opponent?.correctAnswer}
                                            accuracyPercentage={results?.opponent?.accuracy}
                                            speed={results?.opponent?.totalSeconds}
                                            speedPercentage={results?.opponent?.speed}
                                            lifelinesUsed={results?.opponent?.lifelieUsed}
                                            lifelinesUsedPercentage={results?.opponent?.lifelinePercentage}
                                        />
                                }
                            </div>
