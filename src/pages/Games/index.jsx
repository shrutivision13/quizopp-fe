    ...
    useEffect(() => {
        if (botUpdateCount < 5) {
            ...
            const interval = setInterval(() => {
                ...
                if (scoreChange === 20) {
                    setBotTotalSeconds(prevTotalSeconds => prevTotalSeconds + intervalDuration / 1000);
                }

                setBotScore(prevScore => {
                    const newScore = prevScore + scoreChange;
                    if (scoreChange === 20) {
                        setBotCorrectAnswers(prevCorrectAnswers => prevCorrectAnswers + 1);
                    }
                    return newScore;
                });
...
const Games = () => {
    ...
    const shuffledOptions = useMemo(() => {
        if (Array.isArray(currentQuestion.options) && typeof currentQuestion.options[0] === 'object') {
    ...
    const handleUseForCoins = useCallback(async () => {
        ...
        if (activeLifelineId == 1) {  // 50:50 Lifeline
            dispatch({ type: 'ACTIVATE_50_50_LIFELINE' });
            dispatch({ type: 'CONFIRM_LIFELINE_USE', id: activeLifelineId, price: cost });
            // resetTimer();
        }
                    ...
                    <div>
                        ...
                        <div className="mt-14 px-20 w-full flex items-center justify-between">
                            <div className="flex">
                                ...
                                <div className="flex items-center justify-center">
                                    ...
                                    <div id="chat-icon" className="ml-8 relative cursor-pointer chat-icon" onClick={() => setOpenEmojiDrawer(true)}>
                        ...
                        <div className="w-full px-20 my-14 h-4 flex items-center justify-center mb-28">
                            <div
                                ...
                                style={{ width: `${centerPosition + playerYouOffset}%`, transition: 'width 2s ease' }}
                            />
                ...
                <div className="bg-CFFFFFF mx-10 my-5 p-10 pt-0 border-1 rounded-10 border-CF1F1F1 shadow-quizCard dark:border-C26284C dark:bg-C20213F relative pb-30">
                    ...
                    <div className="grid mt-30 grid-cols-2 gap-20">
                        {shuffledOptions.map((optionObj, idx) => {
                            ...
                            const isSelectedWrong = selectedOption && text === selectedOption && !isCorrect;

                            return (
                                <div
                                    key={idx}
                                    className={`justify-center py-10 text-14 shadow-quizCard border-1 font-medium rounded-10 bg-CFFFFFF border-CF1F1F1 dark:text-CFFFFFF dark:border-C26284C dark:bg-C26284C px-22 answer-input cursor-pointer flex items-center flex-col select-none text-center min-h-[60px] ${shakeIndex === idx ? 'animate-shake' : ''}`}
                                    style={{
                                    ...
                                    onClick={() => !selectedOption && !hidden && handleAnswer(text, idx)}
                                >
