...
function reducer(state, action) {
    switch (action.type) {
        case 'SET_QUESTIONS': {
            const newQs = [...action.questions.slice(0, 5)];
            const extraQuestion = action.questions[5];
...
const Games = () => {
    ...
    const goToNextOrSubmit = useCallback(() => {
        if (currentIndex < questions.length - 1) {
            ...
            setTimeout(() => {
                ...
                //     .then(res => {
                //         if (res.isSuccess) {
                //             console.log("Payload submitted successfully after navigation", res.data);
                //         }
                //     })
                //     .catch(console.error);
            }, 200);
    ...
    useEffect(() => {
        setTimeout(() => {
            if (currentIndex === questions.length - 1) {
                console.log("if")
                goToNextOrSubmit();
                // For last question, wait slightly longer to ensure state update
    ...
    const handleUseForCoins = useCallback(async () => {
        ...
        activeLifeline,
        state.questions,
        state.extraQuestion, // Ensure we are using the updated extra question from state
        resetTimer,
    ]);
