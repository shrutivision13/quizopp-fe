...
const PhoneLogin = () => {
    ...
    const handleContinue = () => {
        if (isValid) {
            // Simulate API call if needed here
        ...
        <>
            <div>
                <div className="text-center">
                    ...
                    {/* Mobile Input Field */}
                    <div data-testid="phone-login-number-input" className="my-60 mx-auto w-calc60 input">
                        <label
                            htmlFor="mobileNumber"
                            className={`border-b flex px-2 relative items-end cursor-text ${inputBorderClass}`}
                        >
                            <div className="font-medium text-16 pb-4 text-C959595 dark:text-CBAC8FF">+91</div>
                            <input
                                id="mobileNumber"
                                type="tel"
                                maxLength="10"
                    ...
                    <PrimaryButton
                        ...
                        data-testid={isValid ? 'phone-login-button-active' : 'phone-login-button-inactive'}
                        className={`w-calc60 m-auto max-w-360 py-12 text-center inline-block uppercase font-bold text-16 text-CFFFFFF rounded-5 bg-C0DB25B defaultButton px-36 cursor-pointer flex items-center flex-col select-none ${isTouched ? (isValid ? 'opacity-100' : 'opacity-70') : isFocused ? 'opacity-70' : 'opacity-100'
                            }`}
                    />
                ...
                <div className="px-20 mt-30">
                    <div className="text-18 font-black dark:text-CFFFFFF">Play Quiz and Win Coins!</div>
                    <ul className="list-disc mt-10 text-14 px-16">
                        {[
                            ...
                            <li key={idx} className="leading-normal text-C959595 mb-16 dark:text-C8789C3">
                                {benefit}
                            </li>
                        ))}
                    </ul>
