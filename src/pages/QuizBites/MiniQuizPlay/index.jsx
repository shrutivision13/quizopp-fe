...
import { ApiGetQuizBitesQuestions, ApiUpdatePrize } from "../../../api-wrapper/quizbites/ApiQuizbites";
...
const MiniQuizPlay = () => {
    ...
    useEffect(() => {
        if (state)
            ApiGetQuizBitesQuestions({ categoryId: state?.categoryIds }).then((res) => {
                if (res?.isSuccess) {
    ...
    const handleAnswer = (selectedOption, index) => {
        ...
        updatedAnswers[currentQuestionIndex] = { selected: selectedOption, correct: isCorrect };
        ...
        setTimeout(() => {
            if (currentQuestionIndex < questions.length - 1) {
                ...
                const totalCorrect = updatedAnswers.filter(ans => ans?.correct).length;
                const payload = {
                    correctAnswer: totalCorrect
                }
                ApiUpdatePrize(payload).then((res) => {
                    if (res?.isSuccess) {
                        navigate('/mini-quiz-over', { state: { totalCorrect, totalQuestion: questions.length, prize: res } })
                    }
                    else {
    ...
    if (!questions.length) {
        ...
        return <div className="mb-20 text-20 leading-26 font-black text-CFFFFFF h-full flex justify-center items-center">No Quizzes Found!!</div>
    }
    return (
        questions.length &&
        (
                    ...
                    <div>
                        <div className="rounded-10 px-20 py-20 shadow-contestCard bg-CFFFFFF dark:bg-C20213F mb-8">
                            <div className="flex justify-between mb-20">
                                <div>
                                    <p className="text-10 font-semibold text-C959595 dark:text-C8789C3 mb-8">{currentQuestion?.categoryName}</p>
                        ...
                        <div className="flex items-center justify-center gap-3">
                            {questions.map((_, index) => {
                                ...
                                const borderClass = currentQuestionIndex === index ? "h-8 w-8 border border-C4782F4" : "h-4 w-4";
                                let bgClass = currentQuestionIndex !== index ? "bg-CE0E0E0" : ""; // default: unanswered
                                if (answer) {
                                    bgClass = answer.correct ? "bg-C0DB25B" : "bg-CE63737";
                                }

                                return <span key={index} className={`${baseClass} ${bgClass} ${borderClass}`}></span>;
                            })}
                        </div>
                    </div>
                </article>
